...
<div class="flex flex-wrap items-end justify-between gap-4">
    <div>
        <h1 class="font-display text-3xl text-ink-100">Product orders</h1>
    ...
    <nav class="flex flex-wrap gap-1" aria-label="Order filter">
        <?php foreach (['all', 'paid', 'refunded'] as $value): ?>
            <a href="/admin/product-orders?status=<?= Fmt::e($value) ?>"
...
<div class="card mt-6">
    <div class="table-wrap">
        <table class="data-table">
            <thead>
            <tr><th scope="col">#</th><th scope="col">Product</th><th scope="col">Buyer</th>
            ...
            <?php foreach ($orders as $order): ?>
                ...
                <tr>
                    <td class="ident">
                        <a href="/admin/product-orders/<?= (int) $order['id'] ?>" class="hover:text-ember-500">#<?= (int) $order['id'] ?></a>
                    </td>
                    <td class="max-w-[14rem] truncate text-ink-100"><?= Fmt::e((string) $order['product_name']) ?></td>
                    ...
                    <td>
                        <span class="badge <?= $s === 'refunded' ? 'badge-muted' : 'badge-ok' ?>"><?= Fmt::e($s) ?></span>
                    </td>
                </tr>
            <?php endforeach; ?>
            <?php if ($orders === []): ?>
