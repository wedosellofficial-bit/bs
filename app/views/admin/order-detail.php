...
?>
<a href="/admin/orders" class="text-sm text-ink-500 hover:text-ink-200">&larr; Orders</a>

<div class="mt-4 flex flex-wrap items-start justify-between gap-4">
    <h1 class="font-display text-3xl text-ink-100">Order #<?= (int) $order['id'] ?></h1>
    <span class="badge <?= $status === 'complete' ? 'badge-ok' : ($status === 'failed' ? 'badge-failed' : 'badge-pending') ?>">
        <?= Fmt::e($status) ?>
    </span>
</div>
...
<div class="mt-6 grid gap-6 lg:grid-cols-2">
    <section class="card p-5">
        ...
        <dl class="mt-3 space-y-2.5 text-sm">
            <div class="flex justify-between gap-4">
                <dt class="text-ink-500">Item</dt>
                <dd class="text-right text-ink-100"><?= Fmt::e((string) $order['nft_name']) ?></dd>
            </div>
            <div class="flex justify-between gap-4">
                <dt class="text-ink-500">Inscription</dt>
                <dd><?= View::partial('partials/ident', ['value' => (string) $order['token_id'], 'head' => 8, 'tail' => 6, 'label' => 'inscription id']) ?></dd>
            </div>
            <div class="flex justify-between gap-4">
            ...
            <div class="flex justify-between gap-4">
                <dt class="text-ink-500">Payout address</dt>
                <dd><?= View::partial('partials/ident', ['value' => (string) $order['buyer_wallet_address'], 'head' => 10, 'tail' => 8, 'label' => 'payout address']) ?></dd>
            </div>
            <div class="flex justify-between gap-4">
                <dt class="text-ink-500">Transaction</dt>
                <dd><?= View::partial('partials/ident', ['value' => $txid, 'head' => 10, 'tail' => 8, 'label' => 'transaction id', 'href' => $explorerTx]) ?></dd>
            </div>
            <div class="flex justify-between gap-4">
    ...
    <section class="card p-5">
        ...
        <?php if (!in_array($status, ['complete', 'refunded'], true)): ?>
            <div class="mt-5 border-t border-ink-800 pt-4">
                <h3 class="text-sm font-semibold text-ink-100">Refund</h3>
                <p class="mt-1 text-xs text-ink-500">
                    Writes a compensating credit. The original debit stays on the statement.
                </p>

                <form method="post" action="/admin/orders/<?= (int) $order['id'] ?>/refund" class="mt-3 space-y-2">
                    <?= Auth::csrfField() ?>
                    ...
                    <label class="check-row">
                        <input type="checkbox" name="relist" value="1" checked>
                        <span>Return the item to the storefront</span>
                    </label>
