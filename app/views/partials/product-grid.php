...
if ($results['items'] === []): ?>
    <div class="card flex flex-col items-center justify-center px-6 py-20 text-center">
        ...
        <a href="/shop" class="btn btn-secondary btn-sm mt-6">Clear all filters</a>
    </div>
<?php else: ?>
    ...
    <?php if ($results['pages'] > 1): ?>
        <nav class="mt-10 flex items-center justify-center gap-1" aria-label="Pagination">
            ...
            ?>
            <?php if ($page > 1): ?>
                <a href="<?= Fmt::e(View::filterUrl($filters, ['page' => $page - 1], '/shop')) ?>"
                   class="btn btn-secondary btn-sm" rel="prev">Previous</a>
            <?php endif; ?>
            ...
            <?php foreach ($window as $number): ?>
                ...
                <a href="<?= Fmt::e(View::filterUrl($filters, ['page' => $number], '/shop')) ?>"
            ...
            <?php if ($page < $pages): ?>
                <a href="<?= Fmt::e(View::filterUrl($filters, ['page' => $page + 1], '/shop')) ?>"
                   class="btn btn-secondary btn-sm" rel="next">Next</a>
            <?php endif; ?>
        </nav>
    <?php endif; ?>
<?php endif; ?>
